...
import { GameEvent, NewHost, RoomCreated, RoomDeleted, UserCreated, UserJoinedRoom, UserLeftRoom } from "@shared/event"

// Scenario #1: Create, Join, Leave
// 1. A user joins.
// 2. The user creates a room.
// 3. Another user joins the room.
// 4. The first user (host) leaves the room.
// 5. The second user becomes the host.
// 6. The second user leaves the room.
// 7. The room is destroyed.
test("Scenario #1", () => {
    const hub = new Hub()
